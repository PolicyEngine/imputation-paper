...
estimate full conditional distributions nonparametrically. They are the
default in \policyengine{}'s data pipelines \citep{ghenis2018quantile,
woodruff2024enhancing} because they capture nonlinear structure without
...
reweight. Honoring weights therefore requires materializing them into the
training data --- the weighted bootstrap of Section~3. Zero-inflated targets add a
...
distributional. We follow the pinball-loss (quantile-loss) tradition for
conditional calibration \citep{koenker2005quantile, meinshausen2006quantile}
and use Wasserstein distance for marginal fit. For the joint, the evaluation
perspective this paper formalizes --- one latent population, surveys as
partial views of it, candidates scored through each view against held-out
survey samples --- has ancestry in several literatures that have not, to our
knowledge, been assembled into an operational benchmark: the survey-statistics
treatment of multiple sources observing one population
\citep{lohr2017combining}, the missing-data view of fusion as block
missingness on a single file \citep{little2002statistical,
rassler2002statistical}, strictly proper scoring of distributional claims
against realized samples \citep{gneiting2007strictly, szekely2013energy},
classifier two-sample tests \citep{lopezpaz2017revisiting}, and the
precision--recall--density--coverage geometry of generative-model
evaluation \citep{naeem2020reliable}. Population-synthesis practice validates
against input marginals rather than held-out views, and reviews of the field
note the absence of a standard benchmark. Section~3.3 operationalizes the
combination; the reweighting fragility diagnostic of Section~3.4 is, to our
knowledge, new.
