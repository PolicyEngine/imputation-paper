...
\label{sp:app:targets}

This study calibrates to the full materialized target surface \populace{} currently constructs for the
US fiscal build. Every target is a source-backed fact from \policyengine{} \chronicle{} \citep{ledger2026},
the \chronicle{} fact store formerly published as \texttt{arch-data} that holds the source publications and
preserves the provenance of each published value; \populace{} composes calibration targets from those facts. The surface combines national,
state, and congressional-district aggregates across the major tax and transfer families
(Table~\ref{sp:tab:target_sources}). It is reported here in full so the empirical claims can be read against
the exact target system used rather than against a pipeline-wide inventory.

Table~\ref{sp:tab:target_sources} lists each family with its administrative source. The system holds
...
and scored (Section~\ref{sp:sec:oos}).}
\label{sp:tab:target_sources}
\end{table}

\section{Optimization hyperparameters}
\label{sp:app:hyperparameters}

Table~\ref{sp:tab:hyperparameters_full} lists the hyperparameters of the $L_0$ optimization with their
...
Source: the \lzeropython{} Hard Concrete implementation used by \populace{}.}
\label{sp:tab:hyperparameters_full}
\end{table}

\section{Algorithm pseudocode}
\label{sp:app:algorithm}

Algorithm~\ref{sp:alg:l0} presents the $L_0$-regularized calibration procedure.
...
Figure~\ref{sp:fig:cost_accuracy} is retained as a supplemental cost diagnostic from the earlier budget
sweep. The matched full-surface probe's current runtimes are reported in Section~\ref{sp:sec:results}: dense
