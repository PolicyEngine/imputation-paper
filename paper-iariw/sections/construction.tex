\section{The production setting: pipeline, microdata, and targets}
\label{sec:construction}
\label{sp:sec:data}

Both stages of the method operate on one production data structure, so we describe the setting once,
up front, to make the methods that follow concrete: how \populace{} assembles the candidate
microdata, which donor sources contribute which variables, how geographic identity attaches, and what
the administrative target surface contains. Stage one (\autoref{imp:sec:stage}) fills this frame;
stage two (\autoref{sp:sec:stage}) selects and weights its records. The pipeline is the harness that
produces both problems; it is not itself the object of evaluation.
...
\texttt{ec290055}, \chronicle{} facts SHA-256 beginning \texttt{82cd9e87}, and target-registry version
...
further variables from the donors in Table~\ref{sp:tab:donors} where the CPS is thin. Each step adds
...
from the current \populace{} US fiscal registry: 37{,}053 \chronicle{} facts produce 32{,}633 targets after
materialization against the candidate frame. Appendix~\ref{sp:app:targets} lists the target families and
