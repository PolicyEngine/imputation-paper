...
in \populace{},\footnote{\populace{} was first released under the name \textsc{Populace}; frozen artifact, release, and schema identifiers retain the legacy \texttt{populace-}/\texttt{ledger-} prefixes, and this paper cites those identifiers verbatim.} \policyengine{}'s open-source microdata stack
...
data versions through \policyenginepy{}; a companion paper
...
The paper proceeds from the concrete to the general.
\autoref{sec:construction} sets out the production setting both
stages share --- the pipeline, the pooled candidate microdata and its
donor sources, geographic assignment, and the administrative target
surface. \autoref{imp:sec:stage} presents the fill stage: background,
evaluation design, the estimator, and results under the
population-view harness. \autoref{sp:sec:stage} presents the
represent stage: calibration background, the target-informed $L_0$
method, and the full-surface results. \autoref{sec:application}
reports the deployed application, and \autoref{sec:discussion} and
\autoref{sec:conclusion} discuss limitations and conclude. Extended
literature reviews, the Hard Concrete construction, and a
cross-domain benchmark appear in the appendices.
