...
objective loss in Equation~\ref{sp:eq:loss_cal}. The run reported here is a matched full-surface probe rather
...
\subsubsection{The full-surface matched probe}

Figure~\ref{sp:fig:objective_frontier} is the headline result. On the current three-year ASEC support file,
...
\begin{figure}[H]
    ...
    target-weighted calibration loss in Equation~\ref{sp:eq:loss_cal}, evaluated on each method's final
    ...
    \label{sp:fig:objective_frontier}
\end{figure}

Table~\ref{sp:tab:sampling_comparison} reports the same comparison with raw ARE and weight diagnostics. The
...
Table~\ref{sp:tab:paired_comparison} expresses the comparison on the headline loss scale. Relative to dense
...
\subsubsection{Size and concentration}
\label{sp:sec:results-controls}
...
\begin{figure}[H]
    ...
    \label{sp:fig:usability}
\end{figure}

\subsubsection{Accuracy by geography}

Table~\ref{sp:tab:calibration_accuracy} breaks the post-$L_0$ refit diagnostics down by geography. The fit is
...
\subsubsection{Raw-error tails}
\label{sp:sec:results-degenerate}
...
The matched probe took 39.4 minutes for the $L_0$ selection run.\footnote{Wall-clock figures are single-workstation runs; the run manifests pin software versions and seeds but did not record hardware profiles --- a replication gap we disclose here.} The post-$L_0$ refit then added about
...
concentration controls. Figure~\ref{sp:fig:cost_accuracy} in Appendix~\ref{sp:app:cost} plots wall-clock
