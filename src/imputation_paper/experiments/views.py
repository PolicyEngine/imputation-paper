...
* :func:`~imputation_paper.experiments.metrics.energy_distance` -- strictly
  proper joint score (weight-sensitive block);
* :func:`~imputation_paper.experiments.metrics.prdc` -- support/coverage
...
def project_view(
    ...
    non_numeric = [c for c in columns if not pd.api.types.is_numeric_dtype(table[c])]
    if non_numeric:
        raise ValueError(
