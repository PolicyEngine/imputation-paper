...
harness in :mod:`imputation_paper.experiments.views` runs on these):
...
def _as_points(values: np.ndarray) -> np.ndarray:
    ...
    if points.ndim == 1:
        points = points[:, None]
    if points.ndim != 2:
        raise ValueError(f"Points must be 1-D or 2-D, got shape {points.shape}.")
    return points
...
def _resample_to_cap(
    ...
    *,
    ...
    picks = rng.choice(len(points), size=cap, replace=True, p=weights / weights.sum())
...
def classifier_two_sample_auc(
    ...
    *,
    ...
    for train_idx, test_idx in folds.split(x, y):
        ...
        aucs.append(
            float(roc_auc_score(y[test_idx], scores, sample_weight=w[test_idx]))
        )
